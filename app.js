...
showBtn.addEventListener("click", () => {
  if (isUser % 2 !== 0) {
    if (selectedValue.value === "select trait") {
      alert("Please select trait ")
      return
    }
    userPowerFullStat = Number(userCardStats.powerstats[selectedValue.value])
    robotPowerFullStat = Number(robotCardStats.powerstats[selectedValue.value])
...
async function getCard() {
  ...
  while (!cardData) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Error: Failed to fetch card with ID ${id}`)
      }
      ...
      isNonNumber = Object.values(jsonData.powerstats).some((stat) =>
        isNaN(Number(stat))
      )
